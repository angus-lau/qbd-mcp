...
{
    [McpServerTool, Description("Create a bill (accounts payable) in QuickBooks Desktop. Vendor and account names are fuzzy-matched.")]
    public static string CreateBill(
        QuickBooksService qb,
        NameResolver resolver,
        [Description("Vendor name (partial match OK)")] string vendorName,
    ...
    {
        var vendorResult = resolver.ResolveVendor(vendorName);
        ...
        // Resolve account names in line items
        foreach (var item in lineItems)
        {
            var acctResult = resolver.ResolveAccount(item.AccountName);
            if (!acctResult.Success)
                return acctResult.ErrorMessage!;
            item.AccountName = acctResult.ResolvedName;
        }

        var result = qb.SendRequest(req =>
        {
            var bill = req.AppendBillAddRq();
            bill.VendorRef.FullName.SetValue(vendorResult.ResolvedName);
    ...
    [McpServerTool, Description("Pay a bill in QuickBooks Desktop by writing a cheque. Vendor and account names are fuzzy-matched.")]
    public static string PayBill(
        QuickBooksService qb,
        NameResolver resolver,
        [Description("Vendor name (partial match OK)")] string vendorName,
        [Description("TxnID of the bill to pay (from ListBills or CreateBill)")] string billTxnId,
        [Description("Bank account to pay from (partial match OK)")] string bankAccountName,
        ...
        [Description("Optional cheque/reference number")] string? refNumber = null)
    {
        ...
        var acctResult = resolver.ResolveAccount(bankAccountName);
        if (!acctResult.Success)
            return acctResult.ErrorMessage!;
        ...
        var result = qb.SendRequest(req =>
        {
            ...
            payment.PayeeEntityRef.FullName.SetValue(vendorResult.ResolvedName);
            payment.BankAccountRef.FullName.SetValue(acctResult.ResolvedName);
